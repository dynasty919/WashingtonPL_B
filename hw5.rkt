        ...
        [(call? e) (let ([v1 (eval-under-env (call-funexp e) env)]
                     ...
                     (if (not (closure? v1))
                         ...
                         (let* ([func (closure-fun v1)]
                                [envi (closure-env v1)]
                                [namemap (cons (fun-nameopt func) v1)]
                                [argmap (cons (fun-formal func) v2)])
                           (eval-under-env (fun-body func) (if (fun-nameopt func)
                                                               (cons argmap (cons namemap envi))
                                                               (cons argmap envi))))))]                                                                                                  
...
(define (mlet* lstlst e2)
  (cond [(= 1 (length lstlst)) (mlet (car (car lstlst)) (cdr (car lstlst)) e2)]
        [#t (mlet (car (car lstlst)) (cdr (car lstlst)) (mlet* (cdr lstlst) e2))]))

;;3.c
(define (ifeq e1 e2 e3 e4)
  (mlet* (list (cons "_x" (ifaunit (int 0) (int 0) e1)) (cons "_y" (ifaunit (int 0) (int 0) e2)))
...
;;4.a
(define mupl-map
  (fun "f2f" "f" (fun "map" "list" (ifaunit (var "list")
                                            (aunit)
